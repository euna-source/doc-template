...
- 유형이 넷입니다. 위에서 아래로 읽는 **기획(plan)·리서치(research)**, 기간을 훑고 날짜로 찾아가는 **일정표(schedule)** — 국면 카드·고정 2주 달력(오늘 표시)·운영 흐름과 부스팅 두 열·서랍 레이어, 조건으로 걸러 찾는 **목록(catalog)** — 상단에 고정되는 필터 줄·검색·건수·핵심 열만·그룹 접기. `doc-template starter schedule`처럼 뼈대를 받아 씁니다.
